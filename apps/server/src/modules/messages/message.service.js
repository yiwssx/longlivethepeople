...
const realtime = require('../../infrastructure/realtime');
...
const { encodeCursor } = require('./message.cursor');
...
const getMessages = async ({ limit, cursor }) => {
    const databaseReady = await database.waitForConnection(DATABASE_WAIT_MS);
    if (!databaseReady) {
