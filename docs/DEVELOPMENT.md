...
The server connects to MongoDB before it begins listening. The Vite process waits for `GET /readyz` to return successfully before it starts, so browser/API/Socket.IO proxy traffic is not sent to a backend that is still starting.
